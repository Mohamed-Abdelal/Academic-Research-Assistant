...
def search_papers(query: str, use_live: bool = True) -> List[Dict]:
    from sources.paper_database import _keyword_relevance
    ...
    if use_live:
        for r in web_tool.search(query, num_results=3):
            ...
            body = r.get("body", "")[:400]
            rel = _keyword_relevance(query, title, body)
            live_results.append({
                "title": title,
                "authors": "Web Source",
                ...
                "citations": 0,
                "doi": r.get("href", ""),
                "relevance": round(rel * 0.85, 2),
                "source_type": "web",
            })

    paper_results = paper_db.search(query, num_results=5)

    all_results = paper_results + live_results
    seen, unique = set(), []
...
def get_research_response(api_key, query, history, report_format, depth):
    ...
    prompt = PromptTemplate(
        template=(
            ...
            "Date: {date}\n\n"
            "Instructions:\n"
            "1. ONLY cite papers that are genuinely relevant to the user's query\n"
            "2. If a paper is not about the query topic, do NOT cite it or fabricate a connection\n"
            "3. If none of the provided papers match the query well, say so honestly "
            "and provide your general knowledge on the topic instead\n"
            "4. Cite relevant sources using [Author, Year] format\n"
            "5. Present conflicting findings objectively\n"
            "6. Highlight practical implications\n"
            "7. Suggest research gaps\n"
...
def main():
    ...
    <div class="hero">
        ...
        <p>AI-powered research synthesis · LangChain chains · Citation management (APA/MLA/Chicago) · Semantic Scholar + arXiv</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not st.session_state.messages:
            ...
            <div style='text-align:center;padding:40px;color:#64748b;'>
                ...
                <p style='font-size:0.8rem'>Powered by Groq · LLaMA 3.3 70B · LangChain Chains · DuckDuckGo · Semantic Scholar · arXiv</p>
